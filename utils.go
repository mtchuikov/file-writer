...
func (fw *FileWriter) getFileSize(file *os.File) (int64, error) {
	stat, err := file.Stat()
	if err != nil {
...
func (fw *FileWriter) openFile(name string, mode os.FileMode) (*os.File, uint, error) {
	f, err := os.OpenFile(name, fw.flags, mode)
	if err != nil {
		err = errors.Unwrap(err)
		return nil, 0, fmt.Errorf(failedToOpenLogFile, err)
	}
...
func (fw *FileWriter) rotateFile() error {
	...
	postfix := time.Now().Format(fw.backupPostfix)
	...
	f, err := os.OpenFile(name, defaulFileFlags, defaulFileMode)
	if err != nil {
		err = errors.Unwrap(err)
...
func (fw *FileWriter) flushBuf() error {
	...
	fw.size -= uint(bufSize)
	fw.size += uint(fw.wc.count)
